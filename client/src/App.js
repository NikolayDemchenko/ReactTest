...
const App = () => {
	...
	//   JSON.parse(localStorage.getItem("settings"))
	// );
	const {createTag, removeTag, updateTag, classes } = TagFunctions(page, setPage, setSettings);

	const RESTManager = GetRESTManager(page, setPage, setSettings);

	const onClick =
		settings && settings.assignStyleId
			? (node) => updateTag(node.id, 'styleId', settings.assignStyleId)
			: (node) => setSettings((settings) => ({ ...settings, node }));
