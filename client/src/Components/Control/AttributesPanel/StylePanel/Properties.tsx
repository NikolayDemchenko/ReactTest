...
import { addNewPropDown, addNewPropUp, removeThisLevelPropByName as removeProp } from './Function/ObjectManager';
import RenameObjectProperty from './Function/RenameObjectProperty';
import PropertiesPanel from './PropertiesPanel';
import Property from './Property';
import { TJssStyle, TStyleProperty } from '../../../../Types/BaseTypes';
...
const Properties: FC<IFCProperties> = (props) => {
	...
	const panels = propPanels.map((panel, index) => {
		return (
			<PropertiesPanel
				...
				updateStyleData={(panelStyle: TJssStyle) => setValue(panel.name, panelStyle)}
	...
	const thisProps = properties.map((property, index) => {
		...
		const onDrop = (targetProp: TStyleProperty, draggedProp: TStyleProperty, target: string) => {
			// console.log("draggedProp", draggedProp);
			const addNewProp = (foo: Function) =>
				foo(removeProp(panelStyle, Object.keys(draggedProp)[0]), targetProp, draggedProp).obj;
			if (target === 'up') {
