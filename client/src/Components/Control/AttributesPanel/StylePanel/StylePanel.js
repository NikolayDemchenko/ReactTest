...
function StylePanel(props) {
  ...
  const cloneStyle = (data, name, node) => {
    setState((state) => {
      const names = page.styles.map((style) => style.name);
      const clonedStyle = createVariable(data, createUniqueName(name, names));
  ...
  const updateStyleData = (data) =>
    updateStyleById(node.styleId, "data", data, setState);
