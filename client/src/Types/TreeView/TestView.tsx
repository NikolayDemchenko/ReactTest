...
export const StyleToTreeNodes: IStyleToTreeNodeArray = (style) => {
  ...
  Object.keys(style).forEach((name) => {
    if (
      ...
      list.items.push({       
        nodes: StyleToTreeNodes(style[name] as TObject),
      });
    } else {
...
const TreeViewCreator = (
  List: FC,
  Item: FC<TTreeNode>
): FC<{ nodes: TTreeNode[] }> => ({ nodes }) => (
  <List>
    {nodes.map(({ properties, nodes: childs, listTitle: childsTitle }, key) => {
      const TreeView = TreeViewCreator(List, Item);
      return (
        <Item {...{ properties, nodes: childs, listTitle: childsTitle, key }}>
