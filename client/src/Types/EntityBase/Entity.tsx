import React, { Dispatch, FC, JSXElementConstructor, ReactElement, SetStateAction } from 'react';
import { IEntity, IProperty } from './Interfaces';
...
export class Entity implements IEntity {
	...
	view=(entity: { [key: string]: string }): FC => () => {		
		return (
			<div>
				{"Сущность"}
				{/* {this.properties.map((property) => {
					const name = property.name;
					return entity[name] && property.view({ value: entity[name] });
				})} */}
			</div>
		);
	};
	...
	// edit: FC<{
	// 	selectedEntity?: { [key: string]: string };
	// 	selectEntity: Dispatch<SetStateAction<{ [key: string]: string } | undefined>>;
	// 	updateInstance: () => Promise<void>;
	// 	setEditInstanceId: (value: React.SetStateAction<string | undefined>) => void;
	// }> = ({ selectEntity, selectedEntity, updateInstance, setEditInstanceId, children }) => {

	// 	const setValue = (property: { name: string; value: string }) => {
	// 		selectEntity((_object) => {
	...
	// 					setValue,
	// 					value: (selectedEntity && selectedEntity[name]) || undefined,
	// 				});
	// 			})}
	// 			{
	// 				<button
